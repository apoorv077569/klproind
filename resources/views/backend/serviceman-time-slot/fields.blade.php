<div class="form-group row">
    <label class="col-md-2" for="serviceman_id">Provider<span> *</span></label>
    <div class="col-md-10 error-div select-dropdown">
        <select class="select-2 form-control user-dropdown" name="serviceman_id" data-placeholder="Select Provider" required>
            <option class="select-placeholder" value=""></option>
            @foreach ($servicemen as $key => $option)
                <option value="{{ $option?->id }}" sub-title="{{ $option?->email }}" image="{{ $option->getFirstMedia('image')?->getUrl() }}" {{ old('serviceman_id', @$timeSlot?->serviceman_id) === $option->id ? 'selected' : '' }}>
                    {{ $option->name }}
                </option>
            @endforeach
        </select>
