        ...
        <div class="row g-sm-4 g-3">
            ...
            <div class="col-xxl-3 col-xl-4 col-sm-6">
                <div class="booking-details-box">
                    ...
                    <div class="booking-content">
                        @php
                            $serviceman = $childBooking?->servicemen?->first();
                        @endphp
                        @if($serviceman)
                        <div class="booking-profile-box">
                            <div class="profile-image-box">
                                ...
                                @if ($imageUrl)
                                    <img src="{{ $imageUrl }}" class="img-fluid"
                                        alt="{{ $serviceman?->name ?? 'Serviceman Image' }}">
                                @else
                                    <div class="initial-letter">
                                        <span>{{ strtoupper(substr($serviceman?->name ?? 'S', 0, 1)) }}</span>
                                    </div>
                                @endif
                                <div class="mt-3">
                                    <h4>{{ $serviceman?->name }}</h4>
                                    <h5 class="name">{{ $serviceman?->email }}</h5>
                                </div>
                            </div>
                            <ul class="profile-content-box">
                                @if (isset($serviceman?->code) && isset($serviceman->phone))
                                    <li>
                                        <span>{{ __('static.phone') }}:</span>
                                        +{{ $serviceman?->code . ' ' . $serviceman->phone }}
                                    </li>
                                @endif
                                @if (isset($serviceman?->getPrimaryAddressAttribute()?->country?->name))
                                    <li>
                                        <span>{{ __('static.country') }}:</span>
                                        {{ $serviceman->getPrimaryAddressAttribute()->country->name }}
                                    </li>
                                @endif
                                @if (isset($serviceman?->getPrimaryAddressAttribute()?->state?->name))
                                    <li>
                                        <span>{{ __('static.state') }}:</span>
                                        {{ $serviceman->getPrimaryAddressAttribute()->state->name }}
                                    </li>
                                @endif
                                @if (isset($serviceman?->getPrimaryAddressAttribute()?->city))
                                    <li>
                                        <span>{{ __('static.city') }}:</span>
                                        {{ $serviceman->getPrimaryAddressAttribute()->city }}
                                    </li>
                                @endif
                            </ul>
                        </div>
                        @else
                            <div class="text-center py-4">
                                <h6 class="text-muted">{{ __('static.no_serviceman_found') }}</h6>
                            </div>
                        @endif
                    </div>
                </div>
                                    ...
                                    @endif
                                            @if ($commission->serviceman_commissions->isNotEmpty())
                                                ...
                                                @if (!in_array($providerId, $servicemanIds))
                                                    <li class="commission">
                                                    {{ $isServiceman ? __('static.booking.your_commission') : __('static.booking.provider_commission') }}:
                                                    <span>
                                                        {{-- <ul> --}}
