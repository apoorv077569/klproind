...
<div class="tab-content" id="servicetabContent">
    ...
    <div class="tab-pane fade {{ session('active_tab') != null ? '' : 'show active' }}" id="general" role="tabpanel" aria-labelledby="general-tab">
        ...
        <div class="form-group row">
            ...
            <div class="col-md-10 error-div">
                <select class="select-2 form-control" id="zone_id" name="zone_id[]" multiple data-placeholder="{{ __('static.service.select_zone') }}">
                    <option value="selectAll">{{ __('static.selectAll') ?? 'Select All' }}</option>
                    @foreach ($zones as $key => $option)
                        <option value="{{ $key }}"
                             @if (!empty($selected_zones) && in_array($key, $selected_zones)) selected @elseif (old('zone_id') && in_array($key, old('zone_id'))) selected @endif>
                            {{ $option }}
                        </option>
                    @endforeach
                </select>
        ...
        <!--<div class="form-group row">
            ...
            <div class="col-md-10">
                <input class='form-control' type="number" id="required_servicemen" name="required_servicemen" value="{{ old('required_servicemen', $service->required_servicemen ?? '0') }}" placeholder="{{ __('static.service.enter_required_servicemen') }}">
        ...
        <input type="hidden" name="per_serviceman_commission" id="per_serviceman_commission" value="{{ isset($service->per_serviceman_commission) ? $service->per_serviceman_commission : 0 }}">
