...
<div class="tab-content" id="servicemanTabContent">
    <div class="tab-pane fade active show" id="general" role="tabpanel" aria-labelledby="general-tab">
        ...
        <div class="form-group row">
            <label class="col-md-2" for="zone_ids">{{ __('static.zone.name') ?? 'Zone' }}<span> *</span></label>
            <div class="col-md-10 error-div select-dropdown">
                <select class="select-2 form-control" id="zone_ids" name="zones[]" multiple="multiple"
                    data-placeholder="{{ __('static.zone.select-zone') ?? 'Select Zones' }}">
                    @foreach ($zones as $zone)
                        <option value="{{ $zone->id }}"
                            @if (old('zones') && in_array($zone->id, old('zones'))) selected @elseif(isset($serviceman) && in_array($zone->id, $serviceman->zones->pluck('id')->toArray())) selected @endif>
                            {{ $zone->name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="form-group row">
            <label class="col-md-2" for="service_ids">{{ __('static.service.services') ?? 'Services' }}</label>
            <div class="col-md-10 error-div select-dropdown" id="services_wrapper">
                <select class="select-2 form-control" id="service_ids" name="services[]" multiple="multiple"
                    data-placeholder="{{ __('static.service.select_services') ?? 'Select Services' }}">
                    @if(old('services'))
                        @php
                        ...
                        @endforeach
                    @elseif(isset($serviceman))
                        @foreach($serviceman->expertise as $service)
                            <option value="{{ $service->id }}" selected>{{ $service->title }}</option>
                        @endforeach
                    @endif
                </select>
          ...
          $(document).on('change', '.category', function(e) {
                ...
                $('#zone_ids').on('change', function() {
                    ...
                    if (zoneIds && zoneIds.length > 0) {
                        $.ajax({
                            url: "{{ route('backend.serviceman.get-zone-services') }}",
                            type: "POST",
                            data: {
                                _token: "{{ csrf_token() }}",
                                zone_id: zoneIds
