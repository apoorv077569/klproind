        ...
        return (new EloquentDataTable($query))
            ...
            ->editColumn('booking_number', function ($row) {
                if ($row->sub_bookings->count() > 1) {
                    ...
                    $primary_on_click_url = $subBooking ? route('backend.booking.showChild', $subBooking->id) : 'javascript:void(0)';
            ...
            ->editColumn('service_name', function ($row) {
                $subBooking = $row->sub_bookings->first();
                $service = $subBooking?->service?->title ?? 'N/A'; 
                $total = $row->sub_bookings->sum('total');
                if ($row->sub_bookings->count() > 1) { 
                    $route = route('backend.booking.show', $row->id); 
                } else { 
                    $route = $subBooking ? route('backend.booking.showChild', $subBooking->id) : 'javascript:void(0)';
                } 
                return ' 
            ...
            ->editColumn('action', function ($row) {
                if ($row->sub_bookings->count() > 1) {
                    ...
                    $subBooking = $row->sub_bookings->first();
                    $routeName = $subBooking ? 'backend.booking.showChild' : null;
                    $data = $subBooking ?? $row;
                }
                return view('backend.inc.action', [
                    'show' => $routeName,
