...
class ServiceManDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('zone_names', function ($row) {
                return $row->zones->pluck('name')->join(', ') ?: 'N/A';
            })
            ...
            ->rawColumns(['checkbox', 'merged', 'created_at', 'status', 'zone_names']);
    }
    ...
    public function query(User $model): QueryBuilder
    {
        ...
        $query = $model->newQuery()->with('zones');
        ...
        // Zone filtering removed - now filter by zone if needed
        if ($providerIds) {
            $query->whereHas('zones', function ($q) use ($providerIds) {
                $q->whereIn('zones.id', $providerIds);
            });
        }
    ...
    public function html(): HtmlBuilder
    {
        ...
        return $this->builder()
            ...
            ->addColumn(['data' => 'zone_names', 'name' => 'zone_names', 'title' => __('static.zone.name') ?? 'Zone', 'orderable' => false, 'searchable' => false])
