...
public function login(LoginRequest $request)
{
    ...
    dump('Request Email:', $request->email);
    ...
    dump('Auth Attempt Result:', $result);

    if ($result) {

        $loggedUser = Auth::user();

        dump('Logged In User:', $loggedUser);

     if ($user?->role?->name != 'admin') {
    return redirect()->back()->with(['error' => 'Login allowed only for admin']);
}

        dump('Redirecting to HOME');
