...
class ServiceController extends Controller
{
        ...
        try {
            ...
            if (Helpers::isUserLogin()) {
                ...
                if ($roleName == RoleEnum::PROVIDER) {
                    ...
                    $servicePackage = $this->servicePackage->where('status', true); // ->withCount('services')->having('services_count', '>=', 1);
                }
            } else {
                $servicePackage = $this->servicePackage->where('status', true); // ->withCount('services')->having('services_count', '>=', 1);
            }
            ...
            if ($request->zone_ids) {
                $zone_ids = explode(',', $request->zone_ids);
                $servicePackage = $servicePackage->whereHas('zones', function (Builder $zones) use ($zone_ids) {
                    $zones->whereIn('zones.id', $zone_ids);
                });
            }

            if ($request->has('is_featured')) {
                $servicePackage->where('is_featured', $request->is_featured);
            }
    ...
    public function checkAvailability(Request $request)
    {
        try {
            ...
            $isAvailable = $service->zones()->where('zones.id', $request->zone_id)->exists();

            if ($isAvailable) {
                return response()->json([
                    'success' => true,
                    'is_available' => true,
                    'message' => __('Service is available in this zone.')
                ]);
            }
            ...
            return response()->json([
                'success' => true,
                'is_available' => false,
                'message' => __('Service ":service" is not available in your selected location.', ['service' => $service->title])
            ]);
