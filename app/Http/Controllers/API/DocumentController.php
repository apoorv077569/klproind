...
use App\Http\Requests\API\UploadServicemanDocumentRequest;
...
class DocumentController extends Controller
{
    ...
    public function uploadServicemanDocument(UploadServicemanDocumentRequest $request)
    {
        return $this->repository->uploadServicemanDocument($request);
    }
    ...
    public function dropdown()
    {
        try {
            return response()->json([
                'success' => true,
                'data' => Document::where('status', 1)->get(['id', 'title'])
            ]);
        } catch (Exception $e) {
            throw new ExceptionHandler($e->getMessage(), $e->getCode());
        }
