...
class CategoryController extends Controller
{
    ...
    public function index()
    {
        ...
        $categories = $categories->withCount(['services']);
    ...
    {
        $cat = $category;
        $categoriesQuery = $this->repository->withOutParent()->where('category_type', CategoryType::SERVICE)->withCount(['services']);
        if (request()->zone_id) {
            $zone_id = request()->zone_id;
            $categoriesQuery->whereHas('zones', function ($zones) use ($zone_id) {
                $zones->Where('zone_id', $zone_id);
            });
        }
        $categories = $categoriesQuery->get();

        $childsQuery = Category::whereNotNull('parent_id')->where('category_type', CategoryType::SERVICE)->withCount(['services']);
        if (request()->zone_id) {
            $zone_id = request()->zone_id;
            $childsQuery->whereHas('zones', function ($zones) use ($zone_id) {
                $zones->Where('zone_id', $zone_id);
            });
        }
        $childs = $childsQuery->get();
