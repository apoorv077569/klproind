...
class ProviderController extends Controller
{
    ...
    {
        $servicemanIds = User::where('created_by', $provider_id)->pluck('id')->toArray();
        $userIds = array_merge([$provider_id], $servicemanIds);

        $addresses = Address::whereIn('user_id', $userIds)
            ->with('user:id,name')
            ->orderByDesc('is_primary')
            ->get(['id', 'address', 'user_id']);

        $formattedAddresses = $addresses->map(function ($addr) {
            $userName = $addr->user ? $addr->user->name : 'Unknown';
            return [
                'id' => $addr->id,
                'address' => $userName . ' - ' . $addr->address,
            ];
        });
