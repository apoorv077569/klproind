...
class ProviderExport implements FromCollection,WithMapping,WithHeadings
{
    ...
    */
    ...
    {
        $providers = User::role(RoleEnum::SERVICEMAN)->where('status', true)->whereNull('deleted_at');
    ...
    public function map($provider): array
    {
        return [
            ...
            $provider->servicemanWallet?->balance ?? '0.0',
            Helpers::getTotalServicemanBookingsByStatus(BookingStatusReq::PENDING,$provider->id) ?? '0',
            Helpers::getTotalServicemanBookingsByStatus(BookingStatusReq::COMPLETED,$provider->id) ?? '0',
            Helpers::getTotalServicemanBookingsByStatus(BookingStatusReq::CANCEL,$provider->id) ?? '0',
            Helpers::getTotalServicemanBookings($provider->id) ?? '0',
        ];
    }
