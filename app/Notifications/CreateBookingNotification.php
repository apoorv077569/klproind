...
class CreateBookingNotification extends Notification implements ShouldQueue
{
    ...
    public function toMail(object $notifiable): MailMessage
    {
        switch ($this->roleName) {
            ...
            case RoleEnum::SERVICEMAN:
                return $this->toServicemanMail($notifiable);
        }
    }
    ...
    public function toServicemanMail($notifiable): MailMessage
    {
        $content = EmailTemplate::where('slug', 'booking-created-serviceman')->first();
        if (!$content) {
            return (new MailMessage)
                ->subject("New booking #{$this->booking->booking_number} in your zone")
                ->line('A new booking has been placed in your service zone.')
                ->line('Booking Payment Status: '.$this->booking->payment_status)
                ->line('Booking Status: '.$this->booking->booking_status->name)
                ->line('Please check your app for details.');
        }
    ...
    public function toArray(object $notifiable): array
    {
        switch ($this->roleName) {
            ...
            case RoleEnum::ADMIN:
                ...
                break;
            case RoleEnum::SERVICEMAN:
                $message = "A new booking #{$this->booking->booking_number} has been placed in your zone. Open your app to accept it.";
                break;
        }
