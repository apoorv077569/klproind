...
class ServicePackageRepository extends BaseRepository
{
    ...
    // Removed getProviders as per user request to remove provider system
    ...
    public function getZoneServices($request)
    {
        $language = app()->getLocale();
        $query = $this->service->whereNull('parent_id')
                              ->with('media');

        if ($request->filled('zone_ids')) {
            $zoneIds = (array) $request->zone_ids;
    ...
    public function getZoneServicemen($request)
    {
        ...
        $servicemen = User::role('serviceman')
        ...
        return response()->json($servicemen);
    }

    public function isProviderCanCreate()
    {
        return true; // Allowing creation without provider constraints as requested
    }
    ...
    public function store($request)
    {
        ...
        try {
            ...
            if ($this->isProviderCanCreate()) {
                ...
                $parts = explode(' to ', $request->start_end_date, 2);
                if (count($parts) !== 2) {
                    throw new Exception('Invalid date range format. Please select a start and end date.', 422);
                }
                ...
                if ($request->filled('zone_id')) {
                    $service_package->zones()->attach($request->zone_id);
                }

                if ($request->filled('serviceman_id')) {
                    $service_package->servicemen()->attach($request->serviceman_id);
                }
                ...
                $service_package->setTranslation('title', $locale, $request->get('title'));
                $service_package->setTranslation('description', $locale, $request->get('description'));
    ...
    public function update($request, $id)
    {
        ...
        try {
            ...
            if (!is_null($request->start_end_date) && str_contains($request->start_end_date, ' to ')) {
                [$start_date, $end_date] = explode(' to ', $request->start_end_date, 2);
                $start_date = \Carbon\Carbon::createFromFormat('d-m-Y', trim($start_date))->format('Y-m-d');
                $end_date = \Carbon\Carbon::createFromFormat('d-m-Y', trim($end_date))->format('Y-m-d');
                $request->merge(['started_at' => $start_date, 'ended_at' => $end_date]);
            }
            $service_package = $this->model->findOrFail($id);
            $service_package->setTranslation('title', $locale, $request->get('title'));
            $service_package->setTranslation('description', $locale, $request->get('description'));
            $data = Arr::except($request->all(), ['title', 'description', 'start_end_date', 'provider_id']);


            $service_package->update($data);

            if ($request->filled('zone_id')) {
                $service_package->zones()->sync($request->zone_id);
            } else {
                $service_package->zones()->detach();
            }

            if ($request->filled('serviceman_id')) {
                $service_package->servicemen()->sync($request->serviceman_id);
            } else {
                $service_package->servicemen()->detach();
            }
