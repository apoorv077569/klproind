...
class ProviderReportRepository extends BaseRepository
{
    ...
    {
        $providers = $this->model->role(RoleEnum::SERVICEMAN)->where('status', true)->whereNull('deleted_at');
        ...
        foreach ($providers as $provider) {
            $providerReportTable .= "
                <tr>
                    ...
                    <td>".Helpers::getDefaultCurrency()?->symbol . ($provider?->servicemanWallet?->balance ?? 0)."</td>
                    <td>".Helpers::getTotalServicemanBookingsByStatus(BookingStatusReq::PENDING,$provider->id)."</td>
                    <td>".Helpers::getTotalServicemanBookingsByStatus(BookingStatusReq::COMPLETED,$provider->id)."</td>
                    <td>".Helpers::getTotalServicemanBookingsByStatus(BookingStatusReq::CANCEL,$provider->id)."</td>
                    <td>".Helpers::getTotalServicemanBookings($provider->id)."</td>
                </tr>";
        }
