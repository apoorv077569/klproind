...
class ServiceRepository extends BaseRepository
{
        ...
        try {
            ...
            if ($this->isProviderCanCreate()) {
                ...
                if (isset($request->zone_id)) {
                    ...
                    if (in_array('selectAll', $zoneIds)) {
                        $zoneIds = Zone::pluck('id')->toArray();
                    }
                    $service->zones()->attach($zoneIds);
                    // Also save the first zone to the zone_id column for backward compatibility
                    $service->zone_id = $zoneIds[0] ?? null;
                    $service->save();
                }
                ...
                if (!isset($request->service_id) && $request->is_random_related_services == true) {
                    ...
                    if (!empty($categoryIds)) {
                        $rand_service_id = $categoryIds[array_rand($categoryIds)];
    ...
    public function edit($id)
    {
        ...
        return view('backend.service.edit', [
            ...
            'zones' => $this->getZones(),
            'selected_zones' => $service->zones->pluck('id')->toArray(),
        ]);
    }
    ...
    public function update($request, $id)
    {
        ...
        try {
            ...
            $service->setTranslation('meta_title', $locale, $request['meta_title']);
            $service->setTranslation('meta_description', $locale, $request['meta_description']);
            $data = Arr::except($request->all(), ['title', 'description', 'content', 'speciality_description', 'meta_title', 'meta_description','tax_id', 'zone_id']);
            $service->update($data);
    ...
    public function getZoneTaxes($request)
    {
        ...
        if ($zoneIds && in_array('selectAll', (array)$zoneIds)) {
            $zoneIds = Zone::pluck('id')->toArray();
        }
        
        $taxes = Tax::whereIn('zone_id', (array)$zoneIds)->where('status', true)->get();
