...
class ProviderRepository extends BaseRepository
{
    ...
    {
        $dateTimeStr = $request->input('date_time') ?? $request->input('dateTime');

        // Handle comma in date string like "20-Mar-2026,10:30 am"
        if ($dateTimeStr) {
        ...
        // ── Fetch servicemen filtered by zone AND service expertise ─────────────
        // user_expertise_services pivot table links servicemen to services they can perform
        $servicemen = User::role(RoleEnum::SERVICEMAN)
            ->where('status', true)
            ->whereHas('zones', function ($query) use ($zoneId) {
                $query->where('zones.id', $zoneId);
            })
            ->whereHas('expertise', function ($query) use ($serviceId) {
                $query->where('services.id', $serviceId);
            })
            ->with(['timeslot', 'media'])
            ->get();

        // ── Check time-slot availability & booking clashes ──────────────────────
        $availableServicemen = [];
        $requestedDay        = strtoupper($dateTime->format('l'));   // e.g. FRIDAY
        $requestedTime       = $dateTime->format('H:i');              // e.g. 10:30

        foreach ($servicemen as $serviceman) {
            $isAvailable = false;
            
            // Log for debugging
            // echo "Checking Serviceman: " . $serviceman->name . " (ID: " . $serviceman->id . ")\n";

            // 1) Check this serviceman has an active time slot covering requested day & time
            $slot = $serviceman->timeslot;
            if ($slot && $slot->is_active && is_array($slot->time_slots)) {
                ...
                if ($dayEntry && !empty($dayEntry['is_active']) && !empty($dayEntry['slots'])) {
                    $requestedTimeObj = Carbon::createFromFormat('H:i', $requestedTime);

                    foreach ($dayEntry['slots'] as $sTime) {
                        $slotStart = Carbon::createFromFormat('H:i', substr($sTime, 0, 5));

                        // Requested time must fall within 60-minute window of a slot start
                        if ($requestedTimeObj->greaterThanOrEqualTo($slotStart) &&
                            $requestedTimeObj->diffInMinutes($slotStart) < 60) {
                            $isAvailable = true;
                            // echo "   - Time slot match: PASS\n";
                            break;
                        }
                    }
                } else {
                    // echo "   - Time slot for $requestedDay: FAIL (Inactive or empty)\n";
                }
            } else {
                // echo "   - TimeSlot: FAIL (No active timeslot found)\n";
            }

            if (!$isAvailable) {
                continue; // Skip — not working at this time
            }

            // 2) Ensure no existing booking clash at the exact requested date-time
            $hasClash = Booking::whereHas('servicemen', function ($q) use ($serviceman) {
                    $q->where('users.id', $serviceman->id);
                })
                ->where('date_time', $dateTime->toDateTimeString())
                ->exists();

            if ($hasClash) {
                // echo "   - Booking Clash: FAIL (Found clash)\n";
                continue; // Skip — already booked at this slot
            }

            // ✅ This serviceman is available
            $availableServicemen[] = [
                'id'             => $serviceman->id,
                'name'           => $serviceman->name,
                'review_ratings' => $serviceman->review_ratings,
                'profile_image'  => $serviceman->getFirstMediaUrl('image') ?: null,
            ];
        }

        // ── Response ────────────────────────────────────────────────────────────
        if (empty($availableServicemen)) {
            return response()->json([
                'success'         => true,
                'isValidTimeSlot' => false,
                'message'         => 'No serviceman available for this service at the requested time.',
                'data'            => [],
            ]);
        }
        ...
        return response()->json([
            ...
            'message'         => count($availableServicemen) . ' serviceman(s) available.',
            'data'            => $availableServicemen,
        ]);
    }

    public function providerTimeslot($id)
    { 
        $timeSlot = $this->timeslot->where('provider_id', $id)
                                   ->orWhere('serviceman_id', $id)
                                   ->first();
        ...
        return response()->json([
            ...
            'provider' => $timeSlot->provider ? [
                'id' => $timeSlot->provider->id,
                'name' => $timeSlot->provider->name,
            ] : null,
            'serviceman' => $timeSlot->serviceman ? [
                'id' => $timeSlot->serviceman->id,
                'name' => $timeSlot->serviceman->name,
            ] : null,
    ...
    public function storeProviderTimeSlot($request)
    {
        ...
        try {
            ...
            $isServiceman = ($roleName == RoleEnum::SERVICEMAN);
            $query = $isServiceman ? ['serviceman_id' => $userId] : ['provider_id' => $userId];

            // Optional: Upsert logic
            $existing = $this->timeslot->where($query)->first();
            if ($existing) {
                return response()->json([
                    'success' => false,
                ...
                $this->timeslot->create(array_merge($query, [
    ...
    public function updateProviderTimeSlot($request)
    {
        ...
        try {
            ...
            if ($roleName !== RoleEnum::PROVIDER && $roleName !== RoleEnum::SERVICEMAN) {
                return response()->json([
            ...
            $timeSlot = $this->timeslot->where($query)->first();
    ...
    public function getDashboardData()
    {
        ...
        if($roleName == RoleEnum::PROVIDER){
            ...
            $latestBookings = $this->booking->whereNotNull('parent_id')->with('service.media')->whereHas('service', function($q) use ($providerId) {
                $q->where('user_id', $providerId);
            })->whereDoesntHave('providerDeclines', function($q) use ($providerId) {
                $q->where('provider_id', $providerId);
            })->latest('created_at')->take(2)->get();
            ...
            // Fetch Latest Booking Requests (Pending bookings in serviceman's zones)
            $user = User::find($userId);
            $zoneIds = $user?->zones->pluck('id')->toArray() ?? [];
            $latestBookingRequests = $this->booking
                ->whereIn('zone_id', $zoneIds)
                ->where('booking_status_id', Helpers::getbookingStatusIdBySlug(BookingEnumSlug::PENDING))
                ->whereDoesntHave('providerDeclines', function($q) use ($userId) {
            ...
            return response()->json([
                ...
                'chart' => $getchartData,
                'booking' => BookingHomeResource::collection($latestBookings),
                'latestBookingRequests' => BookingHomeResource::collection($latestBookingRequests),
                'latestBlogs' => BlogResource::collection($blogs),
            ]);
        }
    ...
    public function getTotalBookings($roleName)
    {
        ...
        if ($roleName == RoleEnum::PROVIDER) {
            return $bookings->whereHas('service', function($q) {
                $q->where('user_id', Helpers::getCurrentProviderId());
            })->count();
        }
    ...
    public function getCompleteBooking($roleName)
    {
        ...
        if ($roleName == RoleEnum::PROVIDER) {
            return $bookings->whereNotNull('parent_id')->whereHas('service', function($q) {
                $q->where('user_id', Helpers::getCurrentProviderId());
            });
        }
