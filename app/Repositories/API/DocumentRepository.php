...
class DocumentRepository extends BaseRepository
{
    ...
    public function getUserDocuments($request)
    {
        ...
        try {
            ...
            if ($user->UserDocuments()) {
                // Get only the latest record IDs for each document type
                $latestDocumentIds = \App\Models\UserDocument::where('user_id', $user->id)
                    ->selectRaw('MAX(id) as id')
                    ->groupBy('document_id')
                    ->pluck('id');

                $userDocuments = $user->UserDocuments()->whereIn('id', $latestDocumentIds);
    ...
    public function uploadServicemanDocument($request)
    {
        DB::beginTransaction();
        try {
            $userDocument = $this->userDocument::create([
