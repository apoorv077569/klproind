...
class BookingRepository extends BaseRepository
{
    ...
    public function openRequests($request)
    {
        try {
            $servicemanId = Helpers::getCurrentUserId();
            $serviceman = User::with('zones')->find($servicemanId);
            
            if (!$serviceman || $serviceman->roles->first()?->name !== RoleEnum::SERVICEMAN) {
                return response()->json(['success' => false, 'message' => 'Unauthorized access'], 403);
            }

            $zoneIds = $serviceman->zones->pluck('id')->toArray();
            $expertiseIds = $serviceman->expertise->pluck('id')->toArray();

            $bookingsQuery = $this->model->whereIn('zone_id', $zoneIds)
                ->whereIn('broadcast_status', ['PENDING_BROADCAST', 'BROADCASTED'])
                ->where('booking_status_id', Helpers::getbookingStatusIdBySlug(BookingEnumSlug::PENDING))
                ->where(function($pq) {
                    $pq->where('payment_method', 'cash')
                       ->orWhere('payment_status', PaymentStatus::COMPLETED)
                       ->orWhere('payment_status', 'PARTIAL');
                })
                ->where(function ($q) use ($servicemanId, $expertiseIds) {
                    // Case 1: Targeted to this serviceman
                    $q->whereHas('servicemen', function ($sq) use ($servicemanId) {
                        $sq->where('users.id', $servicemanId);
                    })
                    // Case 2: Broadcast (no specific assignment) but matches expertise
                    ->orWhere(function($subQ) use ($expertiseIds) {
                        $subQ->whereDoesntHave('servicemen')
                             ->where(function($expQ) use ($expertiseIds) {
                                 $expQ->whereIn('service_id', $expertiseIds)
                                      ->orWhereNotNull('service_package_id'); // Allow packages for now, or add specific package check
                             });
                    });
                })
                ->whereDoesntHave('providerDeclines', function($q) use ($servicemanId) {
                    $q->where('provider_id', $servicemanId);
                })
                ->latest('created_at');

            // Fetch all matching by zone and expertise, then filter by availability in PHP
            $bookings = $bookingsQuery->get()->filter(function ($booking) use ($serviceman) {
                return $serviceman->isAvailableAt($booking->date_time);
            })->values();

            // Handle manual pagination for the collection
            ...
            return \App\Http\Resources\BookingResource::collection($paginatedItems);
    ...
    public function acceptBooking($request, $id)
    {
        DB::beginTransaction();
        try {
            $servicemanId = Helpers::getCurrentUserId();
            
            // DB Lock to prevent race condition
            $booking = $this->model->where('id', $id)->lockForUpdate()->first();

            if (!$booking) {
                return response()->json(['success' => false, 'message' => 'Booking not found'], 404);
            }

            if ($booking->booking_status_id !== Helpers::getbookingStatusIdBySlug(BookingEnumSlug::PENDING)) {
                return response()->json(['success' => false, 'message' => 'Booking has already been accepted or is no longer available'], 400);
            }

            if ($booking->payment_method != 'cash' && $booking->payment_status != PaymentStatus::COMPLETED && $booking->payment_status != 'PARTIAL') {
                return response()->json(['success' => false, 'message' => 'Payment is pending for this booking'], 400);
            }

            // Assign the serviceman
            $booking->servicemen()->sync([$servicemanId]);
            
            // Update the booking status
            $booking_status_id = Helpers::getbookingStatusIdBySlug(BookingEnumSlug::ACCEPTED);
            $booking->update([
                'booking_status_id' => $booking_status_id,
                'broadcast_status' => 'ACCEPTED_BY_SERVICEMAN'
            ]);
    ...
    public function show($id)
    {
        try {
            ...
            // Generate and send OTP if not present
            if (!$booking->service_start_otp) {
                $allowedStatusSlugs = [
                    BookingEnumSlug::ACCEPTED,
                    BookingEnumSlug::ASSIGNED,
                    BookingEnumSlug::ON_THE_WAY,
                    BookingEnumSlug::ON_GOING
                ];

                $currentStatusSlug = $booking->booking_status?->slug;

                if (in_array($currentStatusSlug, $allowedStatusSlugs)) {
                    $otp = (string) rand(100000, 999999);
                    $booking->update(['service_start_otp' => $otp]);
                    
                    if ($booking->consumer && $booking->consumer->phone) {
                        $countryCode = $booking->consumer->code ?? '91';
                        $sendTo = '+' . $countryCode . $booking->consumer->phone;
                        $message = "Your service start OTP for Booking #{$booking->booking_number} is: {$otp}";
    ...
    public function booking($service, $request)
    {
        ...
        $servicemenIds = $service['servicemen_ids'] ?? $service['serviceman_ids'] ?? $service['selected_servicemen_ids'] ?? [];
        if (!empty($servicemenIds)) {
            ...
            if (isset($request['booking_status'])) {
                ...
                switch ($booking_status?->slug) {
                    ...
                    case BookingEnumSlug::DECLINE:
                        $providerId = Helpers::getCurrentUserId();
                        // Check if it's a broadcast or direct assignment
                        // A decline is global if:
                        // 1. It's done by a Provider (owner)
                        // 2. It's a direct assignment (has servicemen)
                        $roleName = Helpers::getCurrentRoleName();
                        $isDirect = $booking->servicemen()->exists();
                        
                        if ($roleName !== RoleEnum::PROVIDER && !$isDirect) {
                            // Individual Decline for Broadcast - No global status change
                            \App\Models\ProviderBookingDecline::updateOrCreate(
                                ['booking_id' => $booking->id, 'provider_id' => $providerId],
                        ...
                        // Otherwise, it's a Direct Assignment - Global Decline
                        $logData = [
                            'title' => 'Booking Declined',
                            'description' => 'The booking has been declined by the provider.',
                        ];
                        break;
                ...
                // For Direct Decline, record reason in log
                if ($booking_status?->name == BookingEnum::DECLINE) {
                    ...
                    // Also record in individual decline table for history
                    \App\Models\ProviderBookingDecline::updateOrCreate(
                        ['booking_id' => $booking->id, 'provider_id' => Helpers::getCurrentUserId()],
                        ['reason' => $request['reason']]
                    );
                }
    ...
    public function verifyOtp($request)
    {
        try {
            ...
            if ($booking->service_start_otp === $request->otp) {
                $booking->update([
                    'is_otp_verified' => true,
                ]);

                // Update status to ON-GOING using the existing update method
                // to ensure logs and events are handled correctly.
                $onGoingStatusId = Helpers::getbookingStatusIdBySlug(BookingEnumSlug::ON_GOING);
                if ($onGoingStatusId) {
                    $this->update(['booking_status' => BookingEnumSlug::ON_GOING], $booking->id);
                }
