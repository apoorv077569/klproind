...
/*                if (!empty($zone_ids)) {
                    $query->whereHas('zones', function (Builder $zones) use ($zone_ids) {
            ...
            $coupons = cache()->remember('dashboard_coupons_' . implode('_', $zone_ids ?: ['null']), now()->addMinute(), function () use ($zone_ids) {
                ...
                $query = $this->coupon->where('status', true)
                        ->where('is_expired', 0)
                        ...
                        ->select('id', 'code', 'min_spend', 'type', 'amount', 'title');
/*                if (!empty($zone_ids)) {
...
/*                if (!empty($zone_ids)) {
            ...
            $servicePackages = cache()->remember('dashboard_service_packages_' . implode('_', $zone_ids ?: ['null']), now()->addMinute(), function () use ($zone_ids) {
                $query = $this->servicePackage->where('status', true)
                        ->where('is_featured', 1);

/*                if (!empty($zone_ids)) {
                    $query->whereHas('zones', function ($query) use ($zone_ids) {
                        $query->whereIn('zones.id', $zone_ids);
                    });
                }*/

                return $query->select('id', 'hexa_code', 'title', 'price', 'provider_id', 'status')
                        ->withCount('services')->having('services_count', '>=', 1)
					    ->with('media') 
                        ->latest()
                        ->take(4)
                        ->get();
            ...
            $featuredServices = cache()->remember('dashboard_services_' . implode('_', $zone_ids ?: ['null']), now()->addMinute(), function () use ($zone_ids) {
                $query = $this->service->where('is_featured', true)->whereNull('deleted_at')->where('status', true);
                
                if (!empty($zone_ids)) {
                    $query->whereHas('categories.zones', function ($query) use ($zone_ids) {
                        $query->whereIn('zones.id', $zone_ids);
                    });
                }

                return $query->select('id', 'title', 'price', 'type', 'duration', 'duration_unit', 'discount', 'required_servicemen', 'service_rate', 'description', 'user_id', 'status')
                                 ->latest()
                                 ->take(2)
                                 ->with('media', 'user:id,name', 'servicemen:id,name', 'servicemen.media')
                                 ->get();
            });
    ...
    public function getDashboardData2($request)
    {
        try {
            ...
            $home_banner_advertisements = cache()->remember('dashboard_home_banner_advertisements' . implode('_', $zone_ids ?: ['null']), now()->addMinute(), function() use ($currentDate , $request , $zone_ids) {
               $query = $this->advertisement
                ...
                if (!empty($zone_ids)) {
                    $query->where('zone', $request?->zone_ids);
                }

                return $query->select('id', 'banner_type', 'video_link' , 'type' , 'screen' ,'provider_id')->with('media')
                    ->get();
            });

            $home_services_advertisements = cache()->remember('dashboard_home_services_advertisements'. implode('_', $zone_ids ?: ['null']), now()->addMinute(), function() use ($currentDate , $request , $zone_ids) {
                $query = $this->advertisement
                    ->whereDate('start_date', '<=', $currentDate)
                    ->whereDate('end_date', '>=', $currentDate)
                    ->where('type', 'service')
                ...
                return $query->select('id', 'banner_type', 'video_link' , 'type' , 'screen' , 'provider_id')->with('services')
                    ->get();
            });
