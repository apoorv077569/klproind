...
class Helpers
{
    ...
    public static function getCountries()
    {
        ...
        return Country::where('name', 'like', 'India%')
        ->pluck('name', 'id');
    }
    ...
    public static function getTotalAmount($services, $service_packages)
    {
        ...
        foreach ($services as $service) {
            ...
            $subtotal[] = $serviceTotal * $multiplier;
        }
    ...
    public static function getTotalServicemanBookingsByStatus($status, $serviceman_id)
    {
        return Booking::where('booking_status_id', self::getBookingStatusIdByReq($status))?->whereHas('servicemen', function ($query) use ($serviceman_id) {
            $query->where('users.id', $serviceman_id);
        })->whereNotNull('parent_id')->whereNull('deleted_at')?->count();
    }
    ...
    public static function getTotalServicemanBookings($serviceman_id)
    {
        return Booking::whereHas('servicemen', function ($query) use ($serviceman_id) {
            $query->where('users.id', $serviceman_id);
        })->whereNotNull('parent_id')->whereNull('deleted_at')?->count();
    }
